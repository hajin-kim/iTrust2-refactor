...
public class DiagnosesStepDefs extends CucumberTest {
    ...
    public void adminNavigate() {
        ...
        ((JavascriptExecutor) driver)
                .executeScript("document.getElementById('manageICDCodes').click();");
    }
    ...
    public void verifyAddedDiagnosis() {
        ...
        final WebElement newRow =
                driver.findElements(By.name("codeRow")).stream()
                        .filter(x -> x.getAttribute("codeid").equals(after.get(0)))
                        .findFirst()
                        .get();
    ...
    public void checkInvalidAdd() {
        ...
        try {
            ...
            assertTrue(
                    err.getText().contains("Code doesn't meet specifications")
                            || err.getText()
                                    .contains("Description exceeds character limit of 250"));
            after =
                    driver.findElements(By.name("codeRow")).stream()
                            .map(x -> x.getAttribute("codeid"))
                            .collect(Collectors.toList());
    ...
    public void checkFailMessage() {
        ...
        assertTrue(
                driver.getPageSource()
                        .contains("Diagnosis must be associated with a diagnosis code"));
    }
    ...
    public void diagCreated() {
        ...
        diag.setCode(code);
        diag.setNote(
                "Patient should avoid contact with others for first 24 hours and take prescribed"
                        + " antibiotics");
        diag.setVisit(visit);
    ...
    public void patientNavigate() {
        waitForAngular();
        ((JavascriptExecutor) driver)
                .executeScript("document.getElementById('viewDiagnoses').click();");
        waitForAngular();
    }
