...
public class DocumentOfficeVisitStepDefs extends CucumberTest {
    ...
    public void documentOV() {
        ...
        final WebElement houseSmokeElement =
                driver.findElement(
                        By.cssSelector(
                                "input[value=\"" + HouseholdSmokingStatus.NONSMOKING + "\"]"));
        houseSmokeElement.click();

        final WebElement patientSmokeElement =
                driver.findElement(
                        By.cssSelector("input[value=\"" + PatientSmokingStatus.NEVER + "\"]"));
     ...
     * @param weightString The weight of the patient.
     * @param lengthString The length of the patient.
     * @param headString The head circumference of the patient.
     * @param smokingStatus The smoking status of the patient's household.
     * @param note The note that the doctor includes.
     */
    @When(
            "^I fill in information on the office visit for an infant with date: (.+), weight:"
                + " (.+), length: (.+), head circumference: (.+), household smoking status: (.+),"
                + " and note: (.+)$")
    public void documentOVWithSpecificInformation(
        ...
        try {
            final WebElement smoking =
                    driver.findElement(
                            By.cssSelector(
                                    "input[value=\""
                                            + HouseholdSmokingStatus.getName(
                                                    Integer.parseInt(smokingStatus))
                                            + "\"]"));
            smoking.click();
            expectedBhm.setHouseSmokingStatus(
                    HouseholdSmokingStatus.parseValue(Integer.parseInt(smokingStatus)));
        } catch (final Exception e) {
            /*
    ...
    /**
     * Documents an office visit with specific information for patients between 3 and 12
     *
     * @param dateString The current date.
     * @param weightString The weight of the patient.
     * @param heightString The height of the patient.
     ...
     */
    @When(
            "^I fill in information on the office visit for patients of age 3 to 12 with date:"
                    + " (.+), weight: (.+), height: (.+), systolic blood pressure: (.+), diastolic"
                    + " blood pressure: (.+), household smoking status: (.+), and note: (.+)$")
    public void documentOVWithSpecificInformation3To12(
            final String dateString,
            final String weightString,
    ...
    /**
     * Documents an office visit with specific information for patients 12 and over.
     *
     * @param dateString The current date.
     * @param weightString The weight of the patient.
     * @param heightString The height of the patient.
     * @param sys The systolic blood pressure of the patient.
     * @param dia The diastolic blood pressure of the patient.
     * @param houseSmoke The smoking status of the patient's household.
     * @param patientSmoke The smoking status of the patient.
     * @param hdl The patient's HDL levels.
     * @param ldl The patient's LDL levels.
     * @param tri The patient's triglycerides levels.
     * @param note The note entered by the HCP
     */
    @When(
            "^I fill in information on the office visit for people 12 and over with date: (.+),"
                + " weight: (.+), height: (.+), systolic blood pressure: (.+), diastolic blood"
                + " pressure: (.+), household smoking status: (.+), patient smoking status: (.+),"
                + " HDL cholesterol: (.+), LDL cholesterol: (.+), triglycerides: (.+), and note:"
                + " (.+)$")
    public void documentOVWithSpecificInformation12Over(
            final String dateString,
            final String weightString,
            final String heightString,
            final String sys,
            final String dia,
            final String houseSmoke,
            final String patientSmoke,
            final String hdl,
            final String ldl,
            final String tri,
            final String note) {
            ...
            final WebElement smoking =
                    driver.findElement(
                            By.cssSelector(
                                    "input[value=\""
                                            + HouseholdSmokingStatus.getName(
                                                    Integer.parseInt(houseSmoke))
                                            + "\"]"));
            smoking.click();
            expectedBhm.setHouseSmokingStatus(
                    HouseholdSmokingStatus.parseValue(Integer.parseInt(houseSmoke)));
        } catch (final Exception e) {
            /*
             * This means that the element wasn't found, which is expected if we
             * enter an invalid value (as one of the test cases does).
        ...
        try {
            final WebElement smoking =
                    driver.findElement(
                            By.cssSelector(
                                    "input[value=\""
                                            + PatientSmokingStatus.getName(
                                                    Integer.parseInt(patientSmoke))
                                            + "\"]"));
            smoking.click();
            expectedBhm.setPatientSmokingStatus(
                    PatientSmokingStatus.parseValue(Integer.parseInt(patientSmoke)));
        } catch (final Exception e) {
            /*
             * This means that the element wasn't found, which is expected if we
             * enter an invalid value (as one of the test cases does).
    ...
    @When(
            "^I add a prescription for (.+) with a dosage of (.+) starting on (.+) and ending on"
                    + " (.+) with (.+) renewals$")
    ...
    @Given(
            "^I have been prescribed the drug (.+) with a dosage of (.+) starting on (.+) and"
                    + " ending on (.+) with (.+) renewals$")
    public void prescriptionAdded(
            final String drug,
            final String dosage,
            final String startDate,
            final String endDate,
    ...
    @Then(
            "^I see a prescription for (.+) with a dosage of (.+) starting on (.+) and ending on"
                    + " (.+) with (.+) renewals$")
    public void prescriptionVisible(
            final String drug,
            final String dosage,
            final String startDate,
            final String endDate,
