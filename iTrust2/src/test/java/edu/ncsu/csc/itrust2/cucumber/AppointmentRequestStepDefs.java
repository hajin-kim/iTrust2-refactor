...
public class AppointmentRequestStepDefs extends CucumberTest {
    ...
    @Autowired private AppointmentRequestService service;
     ...
     *
     * @param text The text to verify as present
     */
     ...
     * @param hcp The HCP to request
     * @param date The date to request
     * @param time The time to request
     * @param comments The comments for the request
     */
    @And(
            "^I choose to request a medical appointment with type (.+), HCP (.+), date (.+), time"
                    + " (.+), and comments (.+)$")
     ...
     */
    @Then(
            "^The appointment request with type (.+), date (.+), and time (.+) is submitted"
                    + " successfully$")
     ...
     *
     * @param date The date of the appointment request to delete.
     */
     ...
     */
    @Given(
            "^The patient has requested a medical appointment with type (.+), HCP (.+), date (.+),"
                    + " time (.+), and comments (.+)$")
    public void createAppointmentRequest(
        ...
        final String datetime =
                String.format("%s-%s-%sT%s:%s:00.000%s", year, month, day, hour, minute, offset);
    ...
    /**
     * Checks to make sure the patient can view the appointment request submitted.
     *
     * @param type The appointment type
     * @param date The date to request
     * @param time The time to request
     */
    @Then(
            "^The patient can view the appointment request with type (.+), date (.+), time (.+),"
                    + " and status (.+)$")
    public void patientViewAppointmentRequest(
            final String type, final String date, final String time, final String status) {
        waitForAngular();
     ...
     */
    @And(
            "^The HCP selects the appointment request with type (.+), HCP (.+), date (.+), time"
                    + " (.+), and comments (.+)$")
    public void hcpSelectApptRequest(
            final String type,
            final String hcp,
            final String date,
            final String time,
     ...
     */
    @Then(
            "^The appointment request with type (.+), HCP (.+), date (.+), time (.+), and comments"
                    + " (.+) is moved into the upcoming medical appointment column$")
    public void appointmentMovedToUpcoming(
            final String type,
            final String hcp,
            final String date,
            final String time,
    ...
    @Then(
            "^The appointment request is deleted from the update medical appointment requests"
                    + " column$")
